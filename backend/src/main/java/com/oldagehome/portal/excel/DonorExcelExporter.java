 ...
 *
 * Exported columns:
 *  Donor ID | Full Name | Gender | Mobile | Email |
...
public class DonorExcelExporter {
    ...
    public static byte[] exportDonors(List<Donor> donors) throws IOException {
        ...
        // ── Header row ────────────────────────────────────────────────────────
        String[] headers = {
            "Donor ID", "Full Name", "Gender", "Age", "Mobile", "Email",
            "Donation Type", "Amount", "Donation Date", "Payment Method",
            "Transaction ID", "Status", "City", "State", "Remarks"
        };
        ...
        for (Donor d : donors) {
            ...
            row.createCell(7).setCellValue(d.getDonationAmount() != null ? d.getDonationAmount().doubleValue() : 0.0);
            row.createCell(8).setCellValue(d.getDonationDate() != null ? d.getDonationDate().format(dtf) : "");
            row.createCell(9).setCellValue(d.getPaymentMethod() != null ? d.getPaymentMethod() : "");
            row.createCell(10).setCellValue(d.getTransactionId() != null ? d.getTransactionId() : "");
            row.createCell(11).setCellValue(d.getStatus() != null ? d.getStatus().name() : "");
            row.createCell(12).setCellValue(d.getCity() != null ? d.getCity() : "");
            row.createCell(13).setCellValue(d.getState() != null ? d.getState() : "");
            row.createCell(14).setCellValue(d.getRemarks() != null ? d.getRemarks() : "");
        }
