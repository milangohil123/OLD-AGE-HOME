...
public interface FoodScheduleRepository extends JpaRepository<FoodSchedule, Long> {
    ...
    @Query("SELECT COUNT(DISTINCT CASE WHEN fs.donor IS NOT NULL THEN fs.donor.id ELSE fs.manualDonorName END) " +
           "FROM FoodSchedule fs WHERE fs.scheduleDate = :date AND " +
           "(fs.donor IS NOT NULL OR (fs.manualDonorName IS NOT NULL AND fs.manualDonorName <> ''))")
